...
while i < 2:
    ...
    sol = solve_ivp(system_simulator, [i, i+1], system_init, args=(pulse, clock), method='Radau', t_eval=clock, first_step = 0.01, rtol = 1e-1, atol = 1e-1)
    simulation_output = sol.y
    simulation_time = sol.t

    system_init = simulation_output[:,-1]        
    
    # Calculate the pulse peak  by identifying the maximum element
    peak = np.append(peak, np.amax(simulation_output[1,:]))
    
    # Identify the index of the maximum element
    out = simulation_output[1,:]

    index = np.where(simulation_output[1,:] == peak[-1])

    peak_index = np.append(peak_index, index[0])
    #peak_index = np.append(peak_index, peak.index(peak[-1]))
    print(index[0])
    print(peak[-1])
    ...
    for j in range(len(simulation_output[0])):
        ind = ind + 1
        cursor.execute('UPDATE BUFFER SET "zero" = ?, "one" = ?, "two" = ?, "three" = ?, "four" = ?, "five" = ?, "six" = ?, "seven" = ?, "eight" = ?, "nine" = ?, "ten" = ?, "eleven" = ? WHERE id = ?', (simulation_time[j], simulation_output[1,j], simulation_output[3,j], simulation_output[7,j], simulation_output[57,j], simulation_output[117,j], simulation_output[75,j], simulation_output[123,j], simulation_output[205,j], simulation_output[217,j], simulation_output[147,j], simulation_output[225,j], ind) )
    connection.commit()
    connection.close()
