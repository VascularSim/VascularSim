...
def calc():
    ...
    # GENERATION OF PULSE
    # initial value
    pulse_initial = np.array([1., 1.])
        
    pulse_generator = lambda t, x: pulsegen(t, x, clock, it)
    
    #t, x = rungekutta4(pulse_generator, pulse_initial, st, et, dt)
    sol = solve_ivp(pulse_generator, [st, et], pulse_initial, method='Radau', t_eval=np.arange(st, et, dt))
    x = sol.y    
    pu = it.transpose()
    pulse = (pu - x[0, :]) * R1 + x[1, :]
