...
while i < 1:
    ...
    if heart_rate_check[0] != heart_rate_check[1]:
        test_ecg = []
        ecg_index = 40
        ecg = ecg_simulate_ecgsyn( sfecg=1/dt, N=20 , Anoise=0, hrmean = 78, hrstd=1, lfhfratio=0.5, sfint=1/dt, ti=(-70, -15, 0, 15, 100), ai=(1.2, -5, 30, -7.5, 0.75), bi=(0.25, 0.1, 0.1, 0.1, 0.4) )
        for hr_val_update in range(1, len(ecg)):
            ind[0] = ind[0] + 1
            if ecg_index == len(ecg):
                ecg_index = 1
            cursor.execute('UPDATE BUFFER SET "twelve" = ? WHERE id = ?', (ecg[ecg_index], ind[0]) )
            test_ecg.append(ecg[ecg_index])
            ecg_index += 1 
    connection.commit()
    connection.close()
    ...
    connection = sql.connect("vascularsim.db")
    cursor = connection.cursor()    
    for j in range(1, len(simulation_output[0])):
        ind[1] = ind[1] + 1
