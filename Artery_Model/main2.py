...
def integrated_ode(t, x, Vin, clock):
    ...
    xdot[33] = x[32] / C[24] - x[46] / C[24] - x[48] / C[24] - x[50] / C[24] - x[34] / C[24]
    xdot[34] = x[33] / L[39] - Rs[39] * x[34] / L[39] - x[35] / L[39]
    xdot[35] = x[34] / C[39] # to 57 brachial
    xdot[36] = x[29] / L[14] - x[36] * Rs[14] / L[14] - x[37] / L[14]
    xdot[37] = x[36] / C[14] - x[37] / [C[14] * Rp[14]]
    xdot[38] = x[29] / L[12] - x[38] * Rs[12] / L[12] - x[39] / L[12]
    xdot[39] = x[38] / C[12] - x[39] / [C[12] * Rp[12]]
...
sgen = np.zeros(126)
x0_init = np.round(np.linspace(0,10000, num=100000), 1)
i = 0
ind = 0
...
while i < n:
    ...
    cursor.execute("SELECT HR FROM HPN")
    ...
    output_pulse = solve_ivp(pulse_generator, [st, et], [pgen[0,-1], pgen[1,-1]], method='Radau', t_eval=np.linspace(st, et, num=100))
    x = output_pulse.y
    ...
    if i == 0:
        sol = solve_ivp(system_finder, [st, et], sgen, method='Radau', t_eval=np.linspace(st, et, num=100))
        xo = sol.y
    else:
        sol = solve_ivp(system_finder, [st, et], sgen[:,-1], method='Radau', t_eval=np.linspace(st, et, num=100))
        xo = sol.y
    ...
    for j in range(100):
        ind = ind + 1
        cursor.execute('UPDATE BUFFER SET "one" = ? WHERE id = ?', (xo[1,j], ind) )
    connection.commit()
    connection.close()
